...
def revisar_pedido(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'Proyectowebapp/revisar_pedido.html', {'selected_options': selected_options, 'form': form})
...
def modificar_pedido(request):
    if request.method == 'POST':
        selected_options = {
            ...
            'presentacion': request.POST.get('presentacion'),
            'maridaje_recomendado': request.POST.get('maridaje_recomendado'),
            'extra_bordes_queso': request.POST.get('extra_bordes_queso'),
        }

        form = PizzaBuilderForm(initial=selected_options)
        return render(request, 'Proyectowebapp/pedir.html', {'form': form})
